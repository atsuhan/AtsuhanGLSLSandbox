...
void main(void){
  ...
  float r=rand(gl_FragCoord.xy*.523);
  float g=rand(gl_FragCoord.xy*.456);
  float b=rand(gl_FragCoord.xy*.789);
  
  gl_FragColor=vec4(r,g,b,1.);
  
  // GLSL will not let me save without this line right here
  // please uncomment to see the rng
  gl_FragColor=vec4(pos.x/resolution.x,pos.y/resolution.y,0,1.);
}
